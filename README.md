...
async def main() -> None:
    pet = await client.pets.retrievewithmenottest(
        "REPLACE_ME",
    )
    print(pet.id)
...
try:
    client.pets.retrievewithmenottest(
        "REPLACE_ME",
    )
...
client.with_options(max_retries=5).pets.retrievewithmenottest(
    "REPLACE_ME",
)
```
...
client.with_options(timeout=5 * 1000).pets.retrievewithmenottest(
    "REPLACE_ME",
)
```
...
pet = response.parse()  # get the object that `pets.retrievewithmenottest()` would have returned
print(pet.id)
```
