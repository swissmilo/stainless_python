...
async def main() -> None:
    pet = await client.pets.retrieveme(
        "REPLACE_ME",
    )
    print(pet.id)
...
client.with_options(max_retries=5).pets.retrieveme(
    "REPLACE_ME",
)
```
...
client.with_options(timeout=5 * 1000).pets.retrieveme(
    "REPLACE_ME",
)
```
...
pet = response.parse()  # get the object that `pets.retrieveme()` would have returned
print(pet.id)
```
