...
Simply import `AsyncMiloJan22` instead of `MiloJan22` and use `await` with each API call:

```python
import asyncio
from milo_jan_22 import AsyncMiloJan22

client = AsyncMiloJan22()


async def main() -> None:
    pet = await client.pets.retrievewithmeagainnot(
        "REPLACE_ME",
    )
    print(pet.id)
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `milo_jan_22.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `milo_jan_22.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `milo_jan_22.APIError`.
...
try:
    client.pets.retrievewithmeagainnot(
        "REPLACE_ME",
    )
except milo_jan_22.APIConnectionError as e:
    print("The server could not be reached")
    print(e.__cause__)  # an underlying Exception, likely raised within httpx.
except milo_jan_22.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except milo_jan_22.APIStatusError as e:
...
client.with_options(max_retries=5).pets.retrievewithmeagainnot(
    "REPLACE_ME",
)
```
...
client.with_options(timeout=5 * 1000).pets.retrievewithmeagainnot(
    "REPLACE_ME",
)
```
...
You can enable logging by setting the environment variable `MILO_JAN_22_LOG` to `debug`.

```shell
$ export MILO_JAN_22_LOG=debug
```
...
pet = response.parse()  # get the object that `pets.retrievewithmeagainnot()` would have returned
print(pet.id)
```
...
client = MiloJan22(
    # Or use the `MILO_JAN_22_BASE_URL` env var
